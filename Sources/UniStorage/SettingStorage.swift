...
public class UserSettings: NSObject {
    ...
    public struct Storage<T: Codable> {
        ...
        public var wrappedValue: T {
            get {
                ...
                return MMKVAccessor.value(forKey: key.rawValue, defaultValue: defaultValue, mmkv: UserSettings.mmkv)
            }
            set {
                assert(initializedFlag)
    ...
    @objc public static func changeUser(with userId: String?) {
        ...
        if let userId = userId {
            self.mmkv = MMKV(mmapID: "user.\(userId)")
        } else {
            self.mmkv = nil
        }
    }
    ...
    public override class func setValue(_ value: Any?, forUndefinedKey key: String) {
        let errorMessage = "UserSettings: Cannot Set Value <\(String(describing: value))> for UndefinedKey <\(key)>"
        assert(false, errorMessage)
    }
}
